...
def run_experiment(n_jobs=6):
    ...
    canica = CanICA(n_components=n_components, smoothing_fwhm=6.,
                    memory="nilearn_cache",  memory_level=2,
                    verbose=1,
                    n_init=10, threshold=5., random_state=0)
    ...
    map_masker = MultiNiftiMasker().fit(data_filenames)
    ...
    component_filename = Parallel(n_jobs=n_jobs)(
    ...
    for estimator, cur_img in zip(estimators, component_filename):
        plot_stat_map(index_img(cur_img, 0),
                      title="%s" % estimator.__class__.__name__,
                      cut_coords=[-36], display_mode='x', colorbar=False)
        plt.savefig("%s.pdf" % estimator.__class__.__name__)
    plt.show()
