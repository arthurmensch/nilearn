...
# TODO: add tests
def fetch_hcp_rest(data_dir=None, n_subjects=10, fetch='ssh'):
    filenames = []
    ...
    A = np.array([0, 103, 206, 308, 410])
    k = np.searchsorted(A, n_subjects)-1
    rest = n_subjects - np.cumsum(A)
    ids = ids[:(k+1)]
    ids[k] = ids[k][:rest[k]]

    source_dir = _get_dataset_dir('HCP', data_dir)

    for subset_dir, subject_ids in zip(['S500-1', 'S500-2', 'S500-3', 'S500-4'], ids):
        for subject_id in subject_ids:
